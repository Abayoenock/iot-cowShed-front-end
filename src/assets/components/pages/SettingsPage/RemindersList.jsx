...
const ReminderList = () => {
  ...
  const urlDelete = `${
    import.meta.env.VITE_REACT_API_URL
  }/api/requestData.php?t=delete&tbl=timesettings&col_Name=timeID&dataID=${selectedVehicleDelete}`
          ...
          <FontAwesomeIcon
            ...
            onClick={() => {
              const user = vehicleData.filter((user) => {
                return user.timeTableID === row.timeID
              })
              setSelectedVehicleDelete(row.timeID)
                ...
                <DialogContent>
                  <DialogContentText id="alert-dialog-slide-description">
                    <span className="text-sm text-lightBlack">
                      Are you sure you want to delete the feeding time set at
                      {selectedVehicle?.time} from the system Keep in mind that
